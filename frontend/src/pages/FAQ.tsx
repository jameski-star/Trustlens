            ...
            {faqs.map((faq, i) => (
              <Card key={i}>
                <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full flex items-center justify-between text-left">
                  ...
                  <ChevronDown className={`w-5 h-5 text-[#475569] flex-shrink-0 transition-transform ${openIndex === i ? 'rotate-180' : ''}`} />
                </button>
                {openIndex === i && <p className="mt-3 text-sm text-[#475569] leading-relaxed">{faq.a}</p>}
              </Card>
            ))}
          </div>
